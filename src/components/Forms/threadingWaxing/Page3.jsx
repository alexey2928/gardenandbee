...
const Page3 = ({ currentStep, goToPreviousPage, goToNextPage }) => {
  ...
  const threadingWaxingConsents =
    consents?.find((item) => item.id === "threadingWaxing")
      ?.threadingWaxingConsent || [];
  ...
  const onSubmit = async (data) => {
    ...
    dispatch(
      savePageData({
        ...
        data: {
          ...data,
          consents: threadingWaxingConsents,
    ...
    try {
      await submitForm("threadingWaxingForm", fullFormData);
      reset();
      sigRef.current?.clear();
      dispatch(clearForm({ formName: "threadingWaxingForm" }));
  ...
  return (
    <div className="max-w-5xl mx-auto bg-white rounded-2xl shadow">
      ...
      {/* Header */}
      <FormHeader title="THREADING & WAXING" pageName="Consent + Liability" />
        ...
        <div className="p-4 space-y-4">
          ...
          <div className="p-4 border rounded bg-gray-50 h-64 overflow-y-scroll text-sm">
            <ul className="ml-2 list-disc flex flex-col">
              {threadingWaxingConsents.length > 0 ? (
                threadingWaxingConsents.map((consent, index) => (
